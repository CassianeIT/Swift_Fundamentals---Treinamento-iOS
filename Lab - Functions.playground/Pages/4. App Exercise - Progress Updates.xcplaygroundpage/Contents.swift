...
func  progressUpdate (steps: Int, goal: Int) {
    ...
    if percent < 0.1 {
        ...
        print("if steps is less than 10% of goal")
    } else if percent < 0.9 {
...
progressUpdate(steps: 11000, goal: 1000)
/*:
...
func pacing (currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double) {
    if currentDistance == totalDistance {
        print("Keep it up!")
    } else if currentTime != totalDistance && currentTime < goalTime{
        print("You've got to push it just a bit harder!")
    }
    
}
pacing(currentDistance: 2000, totalDistance: 2000, currentTime: 30, goalTime: 10000)
