...
*/
func calcularArea (x: Double, y: Double) -> Double? {
    guard x > 0 && y > 0 else {
        return nil
    }
    return x * y
...
calcularArea(x: 10, y: 10)
calcularArea(x: -1, y: -8)
calcularArea(x: -1, y: 8)
...
func add(x: Int?, y: Int?) -> Int? {
    guard let posibleX = x,
          let posibleY = y else {
        return nil
    }
    // parametros validados pelo Guard
    return posibleX + posibleY
}

let soma = add(x: 2, y: 2)
print(soma)

let errado = add(x: nil, y: 2)
/*:
...
let testeuser = createUser()
// Vamos nos certificar que o usuario foi criado.
if let user = testeuser {
